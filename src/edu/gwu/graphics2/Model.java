...
public class Model
{
	...
	public Model(final String name, final List<Vector> pts, final List<RefPolygon> plys, List<Coord> textCoord, Material mat)
	{
		this(name, pts, plys, new LinkedList<Line>(), textCoord, mat);
	}
	public Model(final String name, final List<Vector> pts, final List<RefPolygon> plys, List<Line> axes, List<Coord> textures, Material mat)
	{
		this(name, pts, plys, axes, new LinkedList<Line>(), mat,						
				new Line(new Vector(0,0,0,0), new Vector(0,0,0,0)), textures);
	}
	
	public Model(final String name, final List<Vector> pts, final List<RefPolygon> plys, List<Line> axes, List<Line> nrmls, Material mat, Line lightVector, List<Coord> textures)
	{
		this.name = name;
	...
	private void buildTexturesCylindrical()
	{
		...
		Model translated = this.multiplyAll(origin);
	 ...
	 * the Z axis
	 */
	public Model cullBackfacingPerspective()
	{
		...
		return new Model(name, points, notCulled, axes, normals, material, lightVector, this.textureCoords);
	}
	...
	public Model multiplyAll(Matrix m)
	{
		...
		Model newM = new Model(name, newPoints, Collections.unmodifiableList(polygons), this.textureCoords, this.material);
		if(axes.size() > 0)
		{
			for(Line axis : axes)
			{
